...
const nextDeliveryDate = (customer:Customer,order?:Order) => {
  const [year,month]=order?.month?.split("-").map(Number)||[];
  const target=year&&month?new Date(year,month-1,1):new Date();
  const day=Math.min(28,Math.max(1,customer.deliveryDayOfMonth||1));
  target.setDate(day);
  if(!order?.month&&target<new Date())target.setMonth(target.getMonth()+1);
...
const closed = (s:string) => ["נסגרה","בוטלה","בוטלה על ידי הלקוח"].includes(s);
...
export default function Home() {
  ...
  const [modal,setModal] = useState<null|"ticket"|"editTicket"|"task"|"close"|"detail"|"password"|"customer"|"newCustomer">(null);
    ...
    if(quote.status==="אושרה"&&(!quote.accountId||!quote.accessInviteEmail)){
      ...
      setToast(result.inviteStatus==="created"
        ...
        : result.inviteStatus==="already-accepted"
          ...
          : "העסקה אושרה וכרטיס הלקוח הוקם. ניתן להוסיף מייל וגישה ללקוח בהמשך.");
      return;
    }
  ...
  return (
    ...
    <div className={`app-shell ${preview?"previewing":""}`} dir="rtl">
      ...
      {modal==="detail"&&<TicketDetailModal
        ticket={store.tickets.find(t=>t.id===selectedTicket)!}
        machine={store.machines.find(m=>m.id===store.tickets.find(t=>t.id===selectedTicket)?.machineId)}
        canEdit={!isStaff&&!closed(store.tickets.find(t=>t.id===selectedTicket)?.status||"")}
        onEdit={()=>setModal("editTicket")}
        onClose={()=>setModal(null)}
      />}
      {modal==="editTicket"&&store.tickets.find(t=>t.id===selectedTicket)&&<TicketEditModal ticket={store.tickets.find(t=>t.id===selectedTicket)!} onClose={()=>setModal("detail")} onSave={(description,contact,phone,resolved)=>{const changedAt=new Date().toISOString();setStore(current=>({...current,tickets:current.tickets.map(ticket=>ticket.id===selectedTicket?{...ticket,description,contact,phone,status:resolved?"בוטלה על ידי הלקוח":ticket.status,updatedAt:changedAt,closedAt:resolved?changedAt:ticket.closedAt,closeReason:resolved?"הלקוח עדכן שהסתדר":ticket.closeReason,events:[...(ticket.events||[]),addTicketEvent(resolved?"הלקוח עדכן שהסתדר וביטל את הקריאה":"הלקוח עדכן את פרטי הקריאה",resolved?"closed":"note")]}:ticket)}));setModal("detail");setToast(resolved?"הקריאה בוטלה":"פרטי הקריאה עודכנו");}}/>}
...
function UserAccessRow({currentUser,customers,user,readOnly,onSave,onRevoke,onPreview}:{currentUser:UserProfile;customers:Customer[];user:UserProfile;readOnly:boolean;onSave:(user:UserProfile,role:Role,accountIds:string[],status:UserProfile["status"])=>Promise<void>;onRevoke:(user:UserProfile)=>Promise<void>;onPreview:(preview:PreviewContext)=>void}){
  ...
  const save=async()=>{setSaving(true);setSaveError("");try{await onSave(user,role,accountIds,status);setEditing(false);setAdminConfirmed(false);}catch(error){setSaveError(firebaseMessage(error));}finally{setSaving(false);}};
  const revoke=async()=>{if(!window.confirm(`להסיר את הגישה של ${user.email}? המשתמש לא יוכל לראות נתוני לקוחות.`))return;setSaving(true);setSaveError("");try{await onRevoke(user);setEditing(false);}catch(error){setSaveError(firebaseMessage(error));}finally{setSaving(false);}};
  const statusLabel=user.status==="active"?"פעיל":user.status==="revoked"?"גישה הוסרה":"ממתין לאישור";
  return <article className="access-user"><div className="access-user-main"><span className="customer-avatar">{(user.displayName||user.email).slice(0,2)}</span><div><strong>{user.displayName||"משתמש"}</strong><small>{user.email}</small></div><Badge>{statusLabel}</Badge><span className="role-label">{user.role==="admin"&&!trustedAdmin?"מנהל לא מאושר":roleNames[user.role]}</span><div className="access-actions"><button disabled={user.status!=="active"||!trustedAdmin&&user.role==="admin"} onClick={()=>onPreview({role:user.role,accountId:user.accountIds[0]||customers[0]?.id||""})}><Eye size={15}/> תצוגה</button><button disabled={readOnly} onClick={()=>{setEditing(value=>!value);setSaveError("");}}><UserCog size={15}/> הרשאות</button>{canRevoke&&<button className="access-revoke" disabled={saving} onClick={()=>void revoke()}><Trash2 size={15}/> הסרת גישה</button>}</div></div>{editing&&<div className="access-editor"><label><span>תפקיד</span><select value={role} onChange={event=>{setRole(event.target.value as Role);setAdminConfirmed(false);setSaveError("");}}><option value="customer">לקוח רגיל</option><option value="multi">לקוח מרובה סניפים</option><option value="service">נציג שירות</option>{canGrantAdmin&&<option value="admin">מנהל מערכת</option>}</select></label><label><span>מצב החשבון</span><select value={status} onChange={event=>{setStatus(event.target.value as UserProfile["status"]);setSaveError("");}}><option value="pending">ממתין לאישור</option><option value="active">פעיל</option>{user.status==="revoked"&&<option value="revoked">גישה הוסרה</option>}</select></label>{(role==="customer"||role==="multi")&&<fieldset><legend>חשבונות לקוח</legend><div>{customers.map(customer=><label key={customer.id}><input type="checkbox" checked={accountIds.includes(customer.id)} onChange={()=>{toggleAccount(customer.id);setSaveError("");}}/><span>{customer.name}</span></label>)}</div></fieldset>}{role==="admin"&&<label className="admin-confirm"><input type="checkbox" checked={adminConfirmed} onChange={event=>setAdminConfirmed(event.target.checked)}/><span>אני מאשר במפורש לתת למשתמש גישת מנהל מלאה</span></label>}{saveError&&<div className="access-save-error"><LockKeyhole size={15}/><span>{saveError}</span></div>}<footer><button onClick={()=>setEditing(false)}>ביטול</button><button className="primary" disabled={saving||(role==="admin"&&!adminConfirmed)||(status==="active"&&(role==="customer"||role==="multi")&&!accountIds.length)} onClick={()=>void save()}>{saving?"שומר…":"שמירת הרשאות"}</button></footer></div>}</article>
}
...
function Orders({orders,isStaff,onChange}:{orders:Order[];isStaff:boolean;onChange:(id:string,data:Partial<Order>)=>void}) {
  const customerName=useCustomerName();
  const customers=useContext(CustomerDirectoryContext);
  ...
  if(!isStaff){const o=orders[0],customer=customers.find(item=>item.id===o.accountId)!;return <><SectionTitle title="הזמנת קפה" sub="ניהול הכמות והתערובת לחודש הבא"/><section className="order-hero"><div><span>מועד האספקה הבא</span><h2>{customer?formatDate(nextDeliveryDate(customer,o)):formatDate(`${o.month}-01`)}</h2><Badge>{o.status}</Badge></div><div className="kg-ring"><strong>{o.requestedKg}</strong><span>ק״ג</span></div></section><OrderEditor order={o} blends={customer?.contractBlends||[]} onSave={onChange}/><section className="panel"><h3>12 חודשים קדימה</h3><div className="month-row">{["אוג׳","ספט׳","אוק׳","נוב׳","דצמ׳","ינו׳","פבר׳","מרץ","אפר׳","מאי","יוני","יולי"].map((m,i)=><div className={i===0?"current":""} key={m}><span>{m}</span><strong>{i===0?o.requestedKg:o.defaultKg} ק״ג</strong><small>{i===0?"ניתן לעריכה":"נעול"}</small></div>)}</div></section></>}
...
function OrderEditor({order,blends,onSave}:{order:Order;blends:string[];onSave:(id:string,d:Partial<Order>)=>void}) {
  const [kg,setKg]=useState(order.requestedKg),[blend,setBlend]=useState(order.blend),[note,setNote]=useState(order.note);
  const allowedBlends=Array.from(new Set([...blends,order.blend].map(item=>item.trim()).filter(item=>item&&item!=="טרם הוגדרה תערובת")));
  const unusual=Math.abs(kg-order.defaultKg)/order.defaultKg>.3;
  return <section className="panel form-panel"><div className="form-grid"><label><span>כמות ברירת מחדל</span><div className="readonly">{order.defaultKg} ק״ג</div></label><label><span>כמות מבוקשת</span><div className="input-suffix"><input type="number" value={kg} onChange={e=>setKg(+e.target.value)}/><b>ק״ג</b></div></label><label><span>תערובת לפי החוזה</span>{allowedBlends.length?<select value={blend} onChange={e=>setBlend(e.target.value)}>{allowedBlends.map(b=><option key={b}>{b}</option>)}</select>:<div className="readonly">יש לפנות למנהל לעדכון התערובות בחוזה</div>}</label><label className="full"><span>הערה להזמנה</span><textarea placeholder="הערה לחלוקה, אספקה או תערובת" value={note} onChange={e=>setNote(e.target.value)}/></label></div>{unusual&&<div className="warning">⚠ השינוי יועבר לאישור הצוות — קיימת חריגה של יותר מ־30% מהכמות הרגילה.</div>}<div className="form-actions"><button className="primary" disabled={!allowedBlends.length} onClick={()=>onSave(order.id,{requestedKg:kg,blend,note,status:"ממתין לאישור"})}>שמירת ההזמנה</button></div></section>
}
...
function Contract({customer,machines}:{customer:Customer;machines:Machine[]}) {return <><SectionTitle title="ההסכם שלי"/><section className="panel contract"><div className="contract-head"><span className="contract-icon">▤</span><div><span>{customer.contractEnd?"הסכם שירות":"הסכם טרם הוגדר"}</span><h2>{customer.name}</h2></div></div><dl><div><dt>סיום ההסכם</dt><dd>{formatDate(customer.contractEnd)}</dd></div><div><dt>כמות חודשית מוסכמת</dt><dd>{customer.monthlyKg||0} ק״ג</dd></div><div><dt>מכונות משויכות</dt><dd>{machines.length} פריטי ציוד</dd></div><div><dt>תערובות בחוזה</dt><dd>{customer.contractBlends?.join(", ")||"טרם הוגדרו"}</dd></div><div><dt>יום אספקה קבוע</dt><dd>{customer.deliveryDayOfMonth||1} בחודש</dd></div></dl></section></>}
...
function TicketModal({customers,accountId,allowAccountChange,preselectedMachine,machines,onClose,onSave}:{customers:Customer[];accountId:string;allowAccountChange:boolean;preselectedMachine:string;machines:Machine[];onClose:()=>void;onSave:(t:Ticket)=>void}) {
  ...
  return <Modal title="פתיחת קריאת שירות" onClose={onClose}>
    <form onSubmit={submit} className="modal-form">
      ...
      <footer><button type="button" onClick={onClose}>ביטול</button><button className="primary" type="submit">פתיחת הקריאה</button></footer>
    </form>
  </Modal>
}
...
function TicketEditModal({ticket,onClose,onSave}:{ticket:Ticket;onClose:()=>void;onSave:(description:string,contact:string,phone:string,resolved:boolean)=>void}) {
  const [description,setDescription]=useState(ticket.description),[contact,setContact]=useState(ticket.contact),[phone,setPhone]=useState(ticket.phone),[resolved,setResolved]=useState(false);
  const submit=(event:FormEvent<HTMLFormElement>)=>{event.preventDefault();onSave(description.trim(),contact.trim(),phone.trim(),resolved);};
  return <Modal title={`עריכת קריאה ${ticket.id}`} onClose={onClose}><form className="modal-form" onSubmit={submit}><div className="form-grid"><label className="full"><span>תיאור הבעיה והמידע המעודכן</span><textarea required maxLength={5000} value={description} onChange={event=>setDescription(event.target.value)}/></label><label><span>איש קשר</span><input maxLength={160} value={contact} onChange={event=>setContact(event.target.value)}/></label><label><span>טלפון לחזרה</span><input maxLength={40} value={phone} onChange={event=>setPhone(event.target.value)}/></label><label className="full"><span><input type="checkbox" checked={resolved} onChange={event=>setResolved(event.target.checked)}/> הסתדרתי — אפשר לבטל את הקריאה</span></label></div><footer><button type="button" onClick={onClose}>ביטול</button><button className="primary" type="submit">שמירת העדכון</button></footer></form></Modal>;
}
...
function TicketDetailModal({ticket,machine,canEdit,onEdit,onClose}:{ticket:Ticket;machine?:Machine;canEdit:boolean;onEdit:()=>void;onClose:()=>void}) {
  const customerName=useCustomerName();
