...
- שירות: המודול הנוכחי בפרויקט זה
  - Firebase Authentication
  - Cloud Firestore בפרויקט `misterbean-platform`
  - הרשאות לפי מסמך משתמש ו-`accountIds`
...
1. אוספי השירות וההרשאות מנוהלים ב-Firestore הייעודי.
2. להוסיף מיפוי בין מזהי הלידים וההצעות הקיימים לבין `accountId`.
3. לשמור על Realtime Database עבור הלידים בזמן תקופת המעבר.
4. לאחד את מעטפת הניווט של הלידים וההצעות עם מודול השירות.
5. להעביר את מסכי הלידים והצעות המחיר בהדרגה לתוך הפלטפורמה.
